...
With this chapter we have addressed the first two research objectives of this dissertation:

\begin{itemize}
    \item[\textbf{RO1}] Highlight the performance gap between current \acrshortpl{HA} signal enhancement under real-world speech in noise  complex acoustic situations.
    \item[\textbf{RO2}] Investigate the potential of \acrshort{DNN}-based \acrshort{SE} to outperform conventional \acrshortpl{HA} processing.
\end{itemize}


We have shown that the signal-enhancement algorithms of five
high-end commercial \acrshortpl{HA} struggle in ecologically valid, complex
acoustic situations reproduced in the studio, to the point of occasionally
degrading performance relative to applying no enhancement at all. It has also shown that \acrshort{DNN}-based \acrfull{SE}
has the potential to outperform them substantially in denoising and objective
intelligibility, and that the causal
...
measured on a \acrshort{KU100} wearing a \acrshort{RIC} \acrshortpl{HA}, the
corresponding Ambisonics-to-binaural \acrshort{BiMagLS} decoder, a synthetic
binaural training corpus for \acrshortpl{HA} \acrshort{SE}, and a reverberant
speech-in-noise test set in Ambisonics and binaural together with the
