...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\columnwidth]{ch03_hearing/figures/phonak_ric.pdf}
  ...
  placed inside the ear canal, which is sealed and held in place by a soft
  silicone \emph{power dome}.}
  \label{fig:lewi_ric}
\end{figure}
...
speech-in-noise scenes, reproduced over an Ambisonics loudspeaker array and captured on a \acrshort{KU100} dummy head,
...
\acrshort{SPL} assumed by default for a unit-\acrshort{RMS} signal.
...
wearing the \acrshortpl{HA}, approximating it with an \acrshort{IIR} filter, and
...
a \acrshort{MUSHRA}-like listening test with two groups of twenty participants
