...
zhang2024urgent}. Again, the standard recipe for injecting acoustic diversity is to convolve clean speech with an \acrshort{RIR}, placing it in a variety of
...
augmentation \citep{ratnarajah2021ts}, so the same should hold for monaural general-purpose \acrshort{SE}. This raises the question of \emph{which} properties of a synthetic \acrshort{RIR} actually matter for \acrshort{SE} generalisation to real acoustics (\RO{3}).
...
trains on the ten-year-old \acrshortpl{RIR} from \acrshort{DNS}5. Generative augmentation of existing
\acrshortpl{RIR} has been explored in \citet{lin2025generative}.
...
we train one \acrshort{SE} model on each one of the RIR datasets (which are summarised in Table \ref{tab:ch4_datasets}) and evaluate it on \emph{real} \acrshortpl{RIR}, both objectively and through a listening test. We find that
...
The \acrshort{DNS}5 baseline has two limitations at the same time: it is
band-limited (rendered at $16\,\text{kHz}$) and uses single-band absorption coefficients. To
...
\begin{figure}[htpb]
  ...
  and the long-dashed line adds the source directivity (SRC+REC+MB).}
...
\end{align}
so that drawing one sample from each band gives a physically plausible reverberation profile. As Figure~\ref{fig:mbrirs_t60dist} shows, samples drawn from these fitted
...
We can also model the directivity of the \emph{receiver} in the
\acrshort{ISM}: when the device is worn on or
near the head, they inherit its acoustic shadow, so that a reflection arriving
from one side is coloured and attenuated depending on its direction of arrival. Our motivation for modelling the head shadow directivity is that beyond its
...
\acrshort{RIR}, reusing the pipeline of the \acrshort{HA} decoder of
...
head. This yields the \textbf{REC+MB}
dataset.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.9\columnwidth,trim={3cm 0cm 3cm 1.5cm},clip]{ch04_mbrirs/figures/speech_directivity.pdf}
...
We note that other mesh and wave-based datasets would combine geometric realism
with a much larger number of rooms, and would therefore be natural candidates for
disentangling realism from room variety. Two prominent examples, which we were
not aware of at the time, are GWA \citep{tang2022gwa}, a large corpus of
wave-solver \acrshortpl{RIR} across thousands of scenes, and the $1{,}000$-room
dataset of \citet{falconperez2025scenewide}, which, although built from
procedurally generated rooms, offers substantial room variety. Evaluating such
sets (or combining them with our \acrshort{ISM} rooms) is a promising
direction we return to in the conclusions (Chapter~\ref{ch98_conclusions}).

\section{Experimental setup}
\label{sec:mbrirs_eval}
Before detailing the setup, Table~\ref{tab:ch4_design} summarises the whole
experimental design: what varies across models (only the training
\acrshortpl{RIR}), and how each model is evaluated. Every test set carries real
room acoustics, so the experiment measures how well models trained on
\emph{synthetic} \acrshortpl{RIR} generalise to \emph{real} ones.
...
\caption{Overview of the experimental design. All six models share identical
clean speech and noise (\acrshort{DNS}5) and differ \emph{only} in the synthetic
\acrshortpl{RIR} used for training (Table~\ref{tab:ch4_datasets}); each is then
evaluated on the same test sets, all carrying \emph{real} acoustics.}
\label{tab:ch4_design}
\begin{tabular}{@{}p{0.25\textwidth} p{0.31\textwidth} p{0.09\textwidth} p{0.22\textwidth}@{}}
\toprule
\textbf{Stage} & \textbf{Data} & \textbf{Acoustics} & \textbf{Metrics} \\
\midrule
\textbf{Training} &
Clean speech + noise (\acrshort{DNS}5), reverberated with one of the six synthetic \acrshort{RIR} sets &
Synthetic & --- \\
\midrule
\textbf{Eval 1:} synthetic mix &
\acrshort{DNS}5 test speech + noise convolved with $397$ real \acrshortpl{RIR} (ACE, MIT, OpenAIR, BUT, OpenSLR28) &
Real & Intrusive + non-intrusive \\
\addlinespace
\textbf{Eval 2:} subjective &
A subset of the Eval 1 outputs, rated by listeners &
Real & \acrshort{MUSHRA} \\
\addlinespace
\textbf{Eval 3:} \textit{headset} &
\acrshort{DNS}5 real noisy recordings, head-worn devices &
Real & Non-intrusive \\
\addlinespace
\textbf{Eval 4:} \textit{speakerphone} &
\acrshort{DNS}5 real noisy recordings, hand-held/desk devices &
Real & Non-intrusive \\
\bottomrule
\end{tabular}
\end{table}
...
a reverberation probability $p_{\text{reverb}} = 1$, so that an \acrshort{RIR} is
...
reverberant input $x$. Two methodological points deserve mention. First, preserving the time alignment
...
use Deep Noise Suppression Mean Opinion Score (DNSMOS, \citealp{reddy2021dnsmos}) and Non-Intrusive Speech Quality Assessment (NISQA)
...
$_{\text{squim}}$). Most follow the \citet{zhang2024urgent}.
...
The results recommend training \acrshort{SE} models on varied,
high-sampling-rate multiband \acrshortpl{RIR}, optionally adding source and
receiver directivity depending on the use case. This answers \RO{3} in the
...
directivity, which does no harm but shows no clear gain. It answers \RO{4}
...
Regarding \RO{3}, extending the \acrshortpl{RIR} with
...
subjective evaluation. Real materials absorb sound unevenly across frequency ---
carpets and curtains absorb high frequencies while reflecting low ones, whereas
hard surfaces such as concrete reflect almost uniformly --- so in a real room the
reverberation time varies substantially with frequency, typically decaying faster
at high frequencies than at low ones. A single-coefficient \acrshort{RIR} imposes
the same decay at all frequencies, producing a flat spectral colouration that no
real room exhibits, whereas multiband absorption reproduces this
frequency-dependent decay, a first-order property of measured responses. This
matters here because the \acrshortpl{RIR} used in evaluation (drawn from the
ACE \citep{eaton2015ace}, MIT \citep{traer2016statistics}, OpenAIR
\citep{murphy2010openair}, BUT ReverbDB \citep{szoke2019building} and OpenSLR28
\citep{ko2017study} databases) were all measured in real rooms, and therefore
carry this frequency-dependent absorption by construction. A model trained on
single-coefficient responses never sees this pattern and generalises worse to
real acoustics, whereas one trained on multiband responses is exposed to it,
which is why frequency-dependent absorption yields the clearest and most
consistent gains. Extending the sampling rate from $16$ to $48\,\text{kHz}$
...
directivity would benefit the head-worn \emph{headset} recordings was not corroborated.

Regarding \RO{4}, geometric realism does not, on its own, compensate for a
...
validity of an \acrshort{RIR} dataset, \emph{coverage} of the space of real rooms
weighs as heavily as the physical fidelity of any single response --- a theme
that runs through this thesis.

One limitation of this answer to \RO{4} is that the mesh-based condition
