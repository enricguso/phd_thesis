...
to minimize the expected loss $\mathcal{L}$ over a \emph{source} domain
$p_S(x, y)$:
\begin{equation}
...
\begin{itemize}{
    \item \textbf{Eurecat-Microson} collaboration project, focusing on the development of a new benchmark for evaluating \acrshort{HAs} and speech denoising algorithms in realistic conditions.
...
processing tasks --- one per partner and field. The remainder of the manuscript tackles the following Research Objectives (RO) and is
...
\begin{itemize}
    ...
    \item \textbf{Chapter~\ref{ch03_hearing}} addresses the \acrshort{HAs} field and is the result of the collaboration with Microson: the developement of an ecologically-avlid \emph{evaluation} for \acrshort{HAs} (\textbf{RO1}), and whether \acrshort{DNN}s are a promising route to generalise better than traditional \acrshort{HAs} algorithms (\textbf{RO2}). Beyond their technical interest, both objectives foster accessibility, and thus inclusivity, for people with hearing loss.
