...
\begin{figure}[htbp]
  ...
  \caption{Schematic of the \emph{audifon lewi R}, a \acrfull{RIC} \acrshortpl{HA}, the form factor used in
  this study. The behind-the-ear unit houses the microphones, battery and processor; a thin
  wire carries the signal to the \emph{receiver}, the miniature loudspeaker
...
\acrshortpl{HA} processing and the \acrshort{DNN}-based enhancement --- to
...
room-acoustic simulation, and rendered as Ambisonics sound fields. In
...
scenes were represented and reproduced as Ambisonics sound fields
...
\emph{restaurant} and \emph{office}. The noise came from the Ambisonic Recordings of Typical Environments (ARTE) database \citep{weisser2019ambisonic},
...
extended to output the field directly in Ambisonics domain. \acrshort{MASP} is the translation from \citet*{politis2016microphone}'s MATLAB library into Python \citep{perez2020parametric}. Working at 10th order, which affords sufficient spatial
...
Each scene was
calibrated to $70\,\text{dB}$ \acrfull{SPL} at the ear position, next to the dummy head, using a
Beyerdynamic MM1 omnidirectional measurement microphone
(Figure~\ref{fig:ha_dummyhead}). We recorded five high-end \acrfull{RIC}
\acrshortpl{HA} available on the market at the time of writing --- the GN ONE
961-DRWC and 561-DRWC, the Phonak Audéo P90-R and P70-R, and the Signia Pure
C\&G 3x --- testing fifteen \acrshortpl{HA}–receiver combinations in total, with
low, mid and high-power receivers for each device. For each recording, the
\acrshortpl{HA} were fitted behind the ears of the \acrshort{KU100} and their receivers
...
putty in addition to the \acrshortpl{HA}' own power dome. Each scene was recorded twice with every \acrshortpl{HA}. In the first pass the
...
\acrshort{KU100} serve as an approximation of what the \acrshortpl{HA}' microphones capture, and are therefore also the input fed to the offline
...
\emph{enabled}, \emph{bypass} post-processed with the non-causal model (\textbf{\acrshort{DNN}}) and \emph{bypass} post-processed with its causal counterpart (\textbf{\acrshort{DNN}-C}). The first
...
did not hold for several \acrshortpl{HA}, whose adaptive processing reacted
differently to the polarity-inverted noise, nor for any of the
\acrshortpl{DNN}, whose non-linear mapping breaks the assumption outright,
...
Because \acrshort{HASPI} and \acrshort{HASQI} rest on a level-dependent
auditory model, they require a calibration relating digital amplitude to
presentation level; both reference and estimate were scaled so that their root
mean square matched the $70\,\text{dB}$ \acrshort{SPL} at which the scenes were
reproduced (Section~\ref{sec:ha_recordings}), rather than the $65\,\text{dB}$
\acrshort{SPL} assumed by default for a signal with Root Mean Square of one (unit-RMS).
...
predictors specific to this domain. \acrshort{HASQI} and \acrshort{HASPI}
were designed to predict, respectively, perceived
\emph{quality} and \emph{intelligibility} for both normal-hearing and
hearing-impaired listeners: each passes the reference and the estimate through
an auditory model that incorporates the listener's audiogram --- here a flat
normal-hearing one --- and compares the two along cepstral and
temporal-envelope features, returning a score in $[0, 1]$ where $1$ denotes a
signal indistinguishable from the reference. \acrshort{MBSTOI}
\citep{andersen2018refinement} is the binaural extension of the
short-time objective intelligibility measure: it correlates the
short-time temporal envelopes of reference and estimate within frequency bands
and across the two ears --- thereby rewarding the preservation of interaural
cues --- and likewise yields a score in $[0, 1]$, monotonically related to
predicted intelligibility. Unlike \acrshort{SI-SDR}, which is a generic
signal-fidelity ratio in decibels, these three are perceptually motivated and
bounded, which is why we report them alongside it.
...
\acrshortpl{HA} are evaluated in. We kept the original splits of both corpora,
...
We trained two variants. \emph{DNN} is the non-causal configuration named \emph{SuDoRM-RF++GC} on their paper, which may look into future samples and
...
had to be reduced from 12 for \emph{\acrshort{DNN}-C} to 2 for \emph{DNN} to fit in \acrshort{GPU}
...
The evaluation presented here has since been complemented by a perceptual study, published at Forum Acusticum Euronoise~2025 \citep{baig2025perceptual}
and carried out by Mart\'i Baig as part of his Master's thesis. Being his
contribution, it lies beyond the scope of this dissertation, and we summarise
it here only for completeness. Using the same recordings and models, it ran
a \acrfull{MUSHRA}-like listening test with two groups of twenty participants
--- \acrshortpl{HA} users with moderate-to-severe hearing loss and normal-hearing
listeners --- rating the six processing conditions (\emph{bypass},
\emph{enabled}, and the causal and non-causal \acrshortpl{DNN}) across the six
scenes, with the \emph{bypass} recording as the hidden reference. Crucially,
it added a \emph{mild} variant of both networks, trained on
\emph{pseudo-anechoic} targets that remove the noise while preserving some
reverberation, in contrast to the \emph{strong} variants studied in this
chapter, whose fully anechoic targets suppress it entirely.

Both groups preferred the mild \acrshort{DNN} models over every other
condition --- the non-causal mild model ranking first, and the causal one, the
only variant that could run on a device, close behind --- while the strong
variants of this chapter were rated lowest of all, criticised for overly
aggressive suppression that sacrificed naturalness and spatial perception. The
\acrshortpl{HA}' own \emph{enabled} processing scored only moderately and with
high variability. Statistically, the robust effect was precisely this
contrast between mild and strong enhancement, rather than a clean separation
between conventional and \acrshort{DNN} processing; and a re-computation of the
objective metrics with each listener's own audiogram broadly tracked these
preferences. Two conclusions carry back to the present work. First, the
subjective results support the objective finding that \acrshort{DNN}
enhancement has the potential to improve on current \acrshortpl{HA} processing in
complex scenes. Second, they indicate that the anechoic reference adopted here,
flagged among the limitations in Section~\ref{sec:ha_discussion}, is indeed
more aggressive than \acrshortpl{HA} users would choose: a pseudo-anechoic target
that leaves some reverberation intact appears to be the more ecologically
sensible design, and a promising direction for the speech-enhancement work of
...
Ambisonic loudspeaker array and capturing them on real devices, we evaluated
five high-end commercial \acrshortpl{HA} under conditions closer to real use, and
compared them against \acrshort{DNN}-based enhancement.
