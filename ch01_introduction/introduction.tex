...
productive, most notably the encoder-decoder structure of the \acrfull{VAE} that underpin latent-diffusion
...
budgets that make web-scale training pay off are incompatible with the real-time,
on-device deployment targets that motivate our work. When scaling the model and the data indiscriminately is not an option,
...
\begin{itemize}{
    ...
    \item \textbf{GuestXR} (Horizon 2020) project, aimed at creating an immersive online social space through extended reality, centred on a machine learning agent called ``The Guest'' designed to facilitate interaction between participants, mitigate conflicts and cyberbullying, support people with communication difficulties, and detect antisocial behaviour. 
