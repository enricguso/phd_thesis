...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth,trim={5cm 6.8cm 5cm 5cm},clip]{ch04_mbrirs/figures/speech_directivity.pdf}
...
\acrfull{SI-SDR} (Section~\ref{sec:ha_metrics}) and the plain \acrshort{SDR}, in
decibels. \emph{Spectral} measures compare their time--frequency
representations: the \acrfull{LSD} and the \acrfull{MCD}, both distances for
...
similarity, and a \acrshort{BERT}-based textual similarity between transcripts.
...
use \acrshort{DNSMOS} \citep{reddy2021dnsmos} and \acrshort{NISQA}
\citep{mittag2021nisqa}, which predict \acrfull{MOS} scores, and the
\acrshort{SQUIM} estimators \citep{kumar2023torchaudio}, which predict
reference-free versions of \acrshort{SI-SDR}, \acrshort{PESQ}, \acrshort{STOI}
and \acrshort{MOS} directly from the noisy signal (denoted with the subscript
...
This chapter has asked which properties of a synthetic \acrshort{RIR} matter for
the generalisation of monaural \acrshort{SE} models to real room acoustics, and
...
important as the complexity of each individual response --- a reminder that,
consistently with the thesis as a whole, the coverage of the training data can
matter as much as the fidelity of any single example.
...
In conclusion, we recommend training \acrshort{SE} models on varied,
$48\,\text{kHz}$ multiband \acrshortpl{RIR}, optionally adding source and
receiver directivity depending on the use case. We release the two most useful
datasets, MB and SRC+REC+MB, as \emph{MB-RIRs}, freely available as research
