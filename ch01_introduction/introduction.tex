...
This data-centric perspective is related to, but distinct from, \emph{curriculum
learning} \citep{bengio2009curriculum}, which keeps the training set fixed and
instead varies the \emph{order} in which examples are presented, from easier to
harder. Here we hold both the model and the training schedule fixed, and act on
the \emph{content} of the data itself --- what it contains and how faithfully it
reflects real conditions --- rather than on the sequence in which it is seen. The
two are complementary facets of a broader shift towards data-centric machine
learning.
...
Concretely, the approach is the same across the three tasks: we first identify the factors that make real test conditions differ from the data a model is usually trained or evaluated on (room acoustics, loudspeaker and receiver characteristics, background noise, and so on), and then either reproduce those factors in the evaluation (Chapter~\ref{ch03_hearing}) or synthesise them into the training data (Chapters~\ref{ch04_mbrirs} and~\ref{ch05_mss}), as faithfully as a tractable, fixed-size dataset allows. The aim is not to match the target distribution exactly but to close the most
consequential gaps between synthetic and real conditions.
